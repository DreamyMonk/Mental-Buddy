  ...
  return (
    <div className={`flex items-start gap-2.5 w-full ${isUser ? 'justify-end' : 'justify-start'}`}>
        <div className="flex-shrink-0 mt-1 self-start">
            {isUser ? (
                 <Image
                    src={userPhotoURL || defaultAvatarUrl} alt="User Avatar" width={30} height={30}
      ...
      <div className={`relative group max-w-[80%] sm:max-w-[70%] md:max-w-[65%] lg:max-w-[60%] ${isUser ? 'order-first mr-1.5' : 'ml-1.5'}`}>
        ...
        {!isUser && !isSecretChat && (
          <div className="absolute bottom-[-18px] left-0 flex items-center gap-1 bg-gray-800/80 backdrop-blur-sm border border-gray-600/60 rounded-full px-1.5 py-0.5 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity duration-150 shadow-lg z-10">
             <button onClick={() => onAction(message.id, 'copy', textContent)} className="p-1.5 rounded-full text-gray-400 hover:text-blue-300 hover:bg-gray-600/50 transition-colors" title="Copy"><FaRegCopy size={13} /></button>
             <button onClick={() => onAction(message.id, 'like')} className={`p-1.5 rounded-full hover:bg-gray-600/50 transition-colors ${ message.reaction === 'like' ? 'text-green-500' : 'text-gray-400 hover:text-green-400' }`} title="Like">{message.reaction === 'like' ? <FaCheckCircle size={13} /> : <FaThumbsUp size={12} />}</button>
             <button onClick={() => onAction(message.id, 'dislike')} className={`p-1.5 rounded-full hover:bg-gray-600/50 transition-colors ${ message.reaction === 'dislike' ? 'text-red-500' : 'text-gray-400 hover:text-red-400' }`} title="Dislike">{message.reaction === 'dislike' ? <FaTimesCircle size={13} /> : <FaThumbsDown size={12} />}</button>
          </div>
        )}
      </div>
    </div>
